...
def process_phonetic(word):
    ...
    word = word.strip()
    return RussianMetaphone(
        reduce_word=False, reduce_phonemes=True, reduce_vowels=False
    ).transform(word)
...
def process_books_metaphone(session) -> bool:
    ...
    try:
        result = session.execute(
            text("SELECT id, name FROM Publication WHERE metaphone IS NULL")
        )
        ...
        try:
            session.execute(
                text("""
                                ...
                                WHERE id = :pub_id
                            """),
                {"metaphone": process_phonetic(name), "pub_id": pub_id},
            )
